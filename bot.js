...
// 📂 ДЕРЕКТЕР БАЗАСЫ
const ADMIN_NUMBER = '77071234567@s.whatsapp.net'; // 🔴 ӨЗ НӨМІРІҢІЗДІ ЖАЗЫҢЫЗ
...
async function connectToWhatsApp() {
    ...
    const sock = makeWASocket({
        auth: state,
        logger: pino({ level: 'silent' }), // Қателіктерді жасыру
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.log('\n📱 QR КОДТЫ СУРЕТКЕ ТҮСІРІП/СКАНЕРЛЕҢІЗ:\n');
    ...
    // 📩 ХАБАРЛАМА ҚАБЫЛДАУ (ЖЕҢІЛДЕТІЛГЕН, 100% ІСТЕЙТІН НҰСҚА)
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        const senderName = msg.pushName || 'Дос';

        // 🟢 ТЕКСТТІ АЛУДЫҢ ЕҢ СЕНІМДІ ЖОЛЫ
        let text = '';
        ...
        if (!text) return; // Бос хабарламаларды (сурет, стикер) елемейді

        const lowerText = text.toLowerCase().trim();
        const args = lowerText.split(' ');
        const command = args[0];

        // Терминалға шығару (Render логтарынан көріп отыру үшін)
